<template>
	<view class="content" v-if="isShow" @click.stop="isShow=false">
		...
		<view style="display: flex;position: absolute;z-index: 999;top:200upx;left: 220upx;">
			<view>
				<image src="../../static/baoyou.png" style="width: 110upx;height: 110upx;margin-right: 20upx;"></image>
			</view>
			<view class="box-name">
				<view class="name" style="width: 100%">
					<text>青果</text><br>
					<text class="total">邀请码:222222</text>
				</view>
			</view>
		</view>
		<view style="position: absolute;z-index: 999;top:320upx;left: 220upx;">
			<view style="color: #fd7d2e;font-size: 30upx;">我在聚多佳品累计赚了</view>
			<view style="color: #f92d60;font-size: 32upx;">0元</view>
		</view>
		<view style="position: absolute;z-index: 999;top:564upx;left: 230upx;">
			<view style="color: #F2F2F2;font-size: 22upx;">2020年9月17日 18时22分数据</view>
		</view>
		<view style="display: flex;justify-content: space-between;flex-wrap: wrap;width: 60%;position: absolute;z-index: 999;top:650upx;left: 150upx;">
			<view style="width: 50%;" v-for="(item,index) of 8" :key='index'>
				<view style="font-size: 24upx;">
					今日收入(元)	</view>
				<view style="color: #f92d60;margin-top: 4upx;">0</view>
			</view>
		
		</view>
		<canvas @click.stop="" :style="{ width: canvasW + 'px', height: canvasH + 'px' }" canvas-id="my-canvas"></canvas>
		<view class="save-btn" @click.stop="saveImage">保存</view>
	</view>
</template>
	...
	export default{
		...
		methods:{
			...
			async __init(){
				...
				//小程序码
			var qrcodeImg = await this.getImageInfo(this.qrcode)
			console.log('eeeeee',qrcodeImg)
			this.ctx.drawImage(qrcodeImg.path,116,468,78, 78)
