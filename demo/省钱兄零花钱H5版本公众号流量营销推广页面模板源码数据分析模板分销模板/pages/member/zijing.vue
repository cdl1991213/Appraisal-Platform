<template>
	<view style="text-align: left">
		<view style="width: 90%;height: 80upx;background-color: #F8F8F8;display: flex;justify-content: space-between;line-height: 80upx;text-align: center;margin:20upx auto;z-index: 120;">
			...
			<view v-for="(item, index) in list" :key="index" class="item">
				<view>
					<view style="margin-bottom: 8upx;text-align: right;">
						<text style="margin-bottom: 8upx;color: #0e80d2" v-if="item.state==1"> 待到账</text>
						<text style="margin-bottom: 8upx;color: #0e80d2" v-if="item.state==2"> 已到账</text>
						<text style="margin-bottom: 8upx;color: #e10a07" v-if="item.state==3"> 失效</text>
					</view>
	...
	export default {
		...
		methods: {
			...
			getMoney(type, isDirectly) {
				...
				let userId = this.$queue.getData('userId');
				this.$Request.getT('/user/selectUserMoneyDetails?page=' + this.page + '&limit=' + this.size + '&userId=' + userId +
