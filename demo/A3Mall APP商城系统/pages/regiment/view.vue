		...
		<mescroll-body ref="mescrollRef" :up="upOption" @init="mescrollInit" @down="downCallback">
			...
			<view v-if="!isLoading">
				...
				<view class="goods-comments clear">
					<view class="title">
						<text>商品评价</text>
						<text v-if="comments.length > 0" @click="$utils.navigateTo('comments/view',{ id: products.goods_id,type: 'regiment' })">更多 &gt;</text>
					</view>
    ...
    methods: {
        ...
        loadGoodsData(){
            this.$http.getRegimentDetail({
				...
				this.url = this.$config.uni_app_web_url + "regiment/view/" + this.goods_id;
        ...
        onBuyClicked(){
			...
			this.$store.dispatch("usersStatus").then(()=>{
				...
				this.$storage.set("VUE_REFERER","/regiment/view/id/" + this.selectedGoodsInfo.id);
...
.goods-price{
    width: 100%;
    height: 140rpx;
    background-image: url(~@/static/images/bg/regiment-bg.png);
